...
{ 
    ...
    /**
     * 问题1: redis
     *     1.redis 的 key $address 是中文，内耗更大并且出现乱码的可能性也是有的。编码字节存储等都是不一样的占用空间
     *     2.redis 未设置过期时间，运维和其他人想启动redis 时，根本不知道该怎么办（个人遇到过）
     *
     * 问题2：
     * 问题3: 访问api 失败没有处理，尽量多使用isset 判断返回参数
     * 问题4：代码逻辑混乱 是我就是会这样拆分，
     *       方法1：调用thrift 接口，
     *       方法2：调用商家坐标返回，
     *       方法3：方法1和方法2执行顺序(执行顺序可以写到config里面去,如果有新增加，直接加一个方法)
     *
     * 问题5：日志应该统一处理，这里用了太多日志的方法，不易维护
     * 问题6: 商家ID 也应该加入到缓存中
     * geo helper 地址转换为坐标
     * @param $address
     * @return bool|string
     ...
     * 回调状态过滤
     *
     * 问题1：代码太啰嗦了，不易维护，不可配置化，不够灵活，如果需要增加一个状态只有修改代码
     * 解决方案：做映射表，如
     * [
     *    900 => 1,
     *    909 => 0,
